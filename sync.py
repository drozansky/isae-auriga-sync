...
def fetch_auriga_schedule():
    ...
    with sync_playwright() as p:
        ...
        # Listen for any schedule JSON or PrimeFaces XML/JSON event payloads
        def handle_response(response):
            try:
                url = response.url.lower()
                if any(k in url for k in ["planning", "schedule", "agenda", "evenement", "event"]):
                    text = response.text()
                    # 1. Look for PrimeFaces regex {"events": [...]}
                    ...
                    # 2. Look for raw JSON response payloads
                    try:
                        data = response.json()
        ...
        # Wait for welcome page to settle
        page.wait_for_timeout(3000)
        print(f"[INFO] Authenticated. Navigating to menuEntry 227...")

        # 3. Navigate directly to your timetable entry
        page.goto(PLANNING_URL, wait_until="networkidle")
        page.wait_for_timeout(4000)

        # 4. Check if there's an iframe embedding the actual agenda
        frames = page.frames
        print(f"[INFO] Total page frames: {len(frames)}")
        for f in frames:
            try:
                # If there's an iframe with the schedule, wait inside it
                f.wait_for_selector(".fc-view, .ui-schedule, div[id*='schedule']", timeout=5000)
                print(f"[INFO] Found schedule in frame: {f.url}")
            except Exception:
                pass

        # Also wait on main page in case it's native DOM
        try:
            page.wait_for_selector(".fc-view, .ui-schedule, div[id*='schedule']", timeout=5000)
            print("[INFO] Schedule found in main frame.")
        except Exception:
            pass

        # Give 5 seconds for background event payloads to be received
        page.wait_for_timeout(5000)
        page.screenshot(path="debug_screen.png")
        browser.close()

    # Deduplicate events by id / start timestamp
    unique_events = []
    seen = set()
    for e in events:
        key = (e.get("id"), e.get("start"), e.get("title"))
        if key not in seen:
            seen.add(key)
...
def parse_event_details(raw_event):
    ...
    if isinstance(start_raw, int) or (isinstance(start_raw, str) and start_raw.isdigit()):
        ...
        end_dt = datetime.fromisoformat(str(end_raw).replace("Z", "+00:00")).astimezone(PARIS_TZ)
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Retrieved {len(raw)} unique events from Auriga.")
    if raw:
